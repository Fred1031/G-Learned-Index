...
private:
    ...
    void pairwise_merge(const Item &new_item,
        ...
        *it++ = new_item;
        ...
        if (has_pgm(target)) {
			std::cerr << "bomb!!! target = " << (int)target << std::endl;
            pgm(target) = PGMType(level(target).begin(), level(target).end());
			std::cerr << pgm(target).segments.size() << std::endl;
			for (int i = 0; i < 5; i ++)
				std::cerr << pgm(target).levels_offsets[i] << std::endl;
			for (int i = 0; i < 19; i ++)
				std::cerr << pgm(target).segments[i].key << ' ';
			std::cerr << std::endl;
		}
    }
...
public:
	...
	PGMType getPGMIndex(int level) {
		if (level < min_index_level) {
			std::cerr << "no level : " << level << std::endl; 
			std::cerr << "min level = " << (int)min_index_level << std::endl;
			exit(0);
		}
		std::cerr << "total = " << levels[0].size() << std::endl;
		std::cerr << pgms[0].levels_offsets[0] << std::endl;
		return pgms[level - min_index_level];
	}

	int get_min_level() { return min_level; }
	int get_used_levels() { return used_levels; }
	bool level_empty(int i) { return level(i).empty(); }
	int get_first_key(int i) { return pgm(i).first_key; }
	int get_offset(int i) { return pgm(i).levels_offsets[1]; } // number of segments in the bottom level
	bool get_has_pgm(int i) { return has_pgm(i); }

	uint32_t get_segment_key(int i, int j) { return pgm(i).segments[j].key; }
	float get_segment_slope(int i, int j) { return pgm(i).segments[j].slope; }
	uint32_t get_segment_intercept(int i, int j)  { return pgm(i).segments[j].intercept; }
    ...
    DynamicPGMIndex(uint8_t base = 8, uint8_t buffer_level = 0, uint8_t index_level = 0)
        : base(base),
          ...
          levels(),
          pgms() {
			  std::cerr << (int)min_level << std::endl;
		...
		std::cerr << buffer_max_size << std::endl;
    ...
    DynamicPGMIndex(Iterator first, Iterator last, uint8_t base = 8, uint8_t buffer_level = 0, uint8_t index_level = 0)
        ...
        *out++ = Item(first->first, first->second);
        ...
        target.resize(std::distance(target.begin(), out));
			...
			std::cerr << used_levels - 1 << std::endl;
			std::cerr << "min _ = " << (int)min_index_level << std::endl;
        if (has_pgm(used_levels - 1)) {
			std::cerr << "built once" << std::endl;
    ...
    iterator find(const K &key) const {
        for (auto i = min_level; i < used_levels; ++i) {
            ...
            if (has_pgm(i)) {
				//std::cerr << "There is pgm" << std::endl;
	...
	//int get_ans() { return 1; }

	iterator g_find(const K &key, int *lo, int *hi) const {
		for (auto i = min_level; i < used_levels; ++i) {
			if (level(i).empty())
				continue;
			...
			if (has_pgm(i)) {
				first = level(i).begin() + lo[i];
				last = level(i).begin() + hi[i];
			}

			/*
			for (int j = first; j < last; j ++) 
				if (query_key == level(i)[j].first)
